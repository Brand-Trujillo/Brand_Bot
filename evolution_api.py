...
def _get_headers():
    if not EVOLUTION_API_KEY:
        raise RuntimeError(
            "EVOLUTION_API_KEY no está configurada. Define la variable de entorno EVOLUTION_API_KEY."
        )
    return {
...
def obtener_respuesta_evolution(prompt, system_prompt=None, max_tokens=512):
    ...
    if not EVOLUTION_API_KEY:
        return (
            "Evolution API no está configurada. "
            "Define la variable de entorno EVOLUTION_API_KEY y revisa EVOLUTION_API_URL."
        )
